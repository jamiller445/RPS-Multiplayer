// multiRPS.js
  ...
  // Initialize Firebase
  var config = {
    apiKey: "AIzaSyDYtz7fiTiILprXOsxYJtw4Bmif8T5AvHs",
    authDomain: "multiplayerrps-df18f.firebaseapp.com",
    databaseURL: "https://multiplayerrps-df18f.firebaseio.com",
    projectId: "multiplayerrps-df18f",
    storageBucket: "multiplayerrps-df18f.appspot.com",
    messagingSenderId: "896526701954"
  };
...
player1.on("value", function(snapshot) {
	if (snapshot.val() !== null) {
        ...
        console.log("p1 wins1 losses1 " + p1 + wins1 + losses1);
...
$("#play").on("click", function(event) {    
	...
	player1.once("value", function(snapshot) {
        ...
        // console.log("p1snanp= "+ JSON.stringify(p1snapshot));
...
var rpsResults = function() {
	...
	if (p1result.val() !== null && p2result.val() !== null) {
		...
		if (p1result.val().choice == p2result.val().choice) {           
            // $("#p1choices").prepend("<br><br><br><h1>" + p1result.val().choice + "</h1>");
            $("#p1choices").html("<h4>" + p1result.val().choice + "</h4>");
            $("#p2choices").html("<h4>" + p2result.val().choice + "</h4>");
			$("#results").html("<h4>Tie Game!</h4>");
		// Player one wins
		} else if (p1result.val().choice == "Rock" && p2result.val().choice == "Scissors") {
			$("#p1choices").html("<h4>" + p1result.val().choice + "</h4>");
			$("#p2choices").html("<h4>" + p2result.val().choice + "</h4>");
			$("#results").html("<h4>" + p1 + " wins!</h4>");
			wins1++;
			losses2++;
		// Player two wins
...
$(".p1-btn").on("click", ".btn", function() {
    var choice = $(this).text();

    console.log("button pressed= " + choice);

	$("#p1choices").html("<h4>" + choice + "</h4>");
	setTimeout(function() {
		playerTurn.update({
			turn: 2
		});
    ...
    $(".p2-btn").on("click", ".btn", function() {
	var choice = $(this).text();
	$("#p2choices").html("<h4>" + choice + "</h4>");
	setTimeout(function() {
		player2.update({
			choice: choice
		}); 
